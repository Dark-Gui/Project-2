...
let makeParser path =
  ...
  let token = ref (Scanner.EndToken) in

    let rec nextThing () =
      match !token with
      | Scanner.CloseParenToken -> raise (CantParse "Unexpected CloseParenToken")
      | Scanner.EndToken -> raise (CantParse "Unexpected EndToken")
      | Scanner.NumberToken n -> token:=scanner(); Number n
      | Scanner.OpenParenToken ->
          token := scanner (); nextThings()
      | Scanner.SymbolToken "nil" -> token := scanner (); Nil
      | Scanner.SymbolToken s -> token := scanner (); Symbol s

    and nextThings () =
      match !token with
      | Scanner.CloseParenToken -> token := scanner (); Nil
      | Scanner.EndToken -> raise (CantParse "List ended without CloseParenToken")
      | _ ->
            let first = nextThing () in
            let rest = nextThings () in
            Cons(first, rest)
    in
    token := scanner (); nextThing;; 
end;;